...
@router.post("/", response_model=LivreResponse)
def ajouter_livre(
    livre: LivreCreate,
...
def supprimer_livre(
    ...
    livre = delete_livre(db, id_livre)
...
@router.put("/{id_livre}", response_model=LivreResponse)
def modifier_livre(
    id_livre: int,
    livre_data: LivreUpdate,
    db: Session = Depends(get_db)
):
    livre = update_livre(db, id_livre, livre_data)
